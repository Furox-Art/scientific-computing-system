function bf = bayesfactor_R_wrapper_2sample(dataX, dataY, varargin)
    ...
    bfstatsinfnX = [bfstatstempdir 'inX.csv'];
    bfstatsinfnY = [bfstatstempdir 'inY.csv'];
    bfstatsoutfn = [bfstatstempdir 'out.csv'];
    bfscriptfn = [bfstatstempdir 'bf_fun.r'];

    % write data to csv file
    %writematrix(data,bfstatsinfn);
    writetable(array2table(dataX),bfstatsinfnX,'WriteVariableNames',0);
    writetable(array2table(dataY),bfstatsinfnY,'WriteVariableNames',0);

    %%
    fid = fopen(bfscriptfn,'w');
    fprintf(fid,['library("BayesFactor")\n'...
        'X = as.matrix(read.table("%s",header=FALSE,sep=","))\n'...
        'Y = as.matrix(read.table("%s",header=FALSE,sep=","))\n'...
        'bf10 = numeric(length = nrow(X))\n'...
        'for (row in 1:nrow(X)) {\n'...
        'dataX = as.numeric(X[row,])\n'...
        'dataY = as.numeric(Y[row,])\n'...
        'bf = ttestBF(x=dataX, y=dataY, %s)\n'...
        'print(bf)\n',...
        'bf10[row] = as.vector(bf[%i])}\n'...
        'write.table(bf10, "%s", sep=",", row.names=FALSE, col.names="BF10")\n'],...
         bfstatsinfnX, bfstatsinfnY, opt.args,opt.returnindex,bfstatsoutfn);
    fclose(fid);
